...
let createNetwork inputSize hiddenSizes outputSize  = 
    let activation = NNetParts.sigmoidActivation
...
let propagateBack learnRate net target =
    let adjustWeightsBetweenLayers inputLayer layer =
        ...
        |> Array.iter (fun neuron ->
            inputLayerValues |> Array.iteri (fun index inputValue->
                let weightDelta = learnRate * neuron.Error * inputValue
